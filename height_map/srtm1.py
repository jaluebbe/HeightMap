...
class Srtm1:
    ...
    attributions = [
        attribution,
        # optional, if using Sonny's data:
        # '&copy <a href="https://sonny.4lima.de/">Sonny</a>',
    ]
    ...
    def __init__(self, path=None, cache_path=None, cache_file_name=None):
        ...
        if cache_file_name is None:
            cache_file_name = os.path.join(
                self.cache_path, "srtm1_map_cache.json"
            )
    ...
    def create_filelist(self, lat_ll, lon_ll, lat_ur, lon_ur, file_list):
        ...
        if lat_ur < y_ll_tile + 1 + CELLSIZE / 2:
            ...
            self.create_filelist(
                y_ll_tile + 1 + CELLSIZE / 2,
                lon_ll,
                lat_ur,
                min(lon_ur, x_ll_tile + 1),
                file_list,
            )
            i_ur = 0
        ...
        if os.path.isfile(full_path):
            ...
            raise IOError(
                f"requested {file_name} is out of SRTM1 coverage or "
                "file is missing."
            )
    ...
    def get_min_max_height(self, lat_ll, lon_ll, lat_ur, lon_ur):
        ...
        try:
            ...
            return result
        result.update(
            self.check_min_max_files(self.filter_min_max_list(file_list))
        )
        return result
